...
| ~~Q-2~~ | ~~日本語の分割方式~~ | **決着（[ADR 0021](adr/0021-q2-bigram-stays-default.md)・2026-09-02）**。既定は **bigram のまま**。形態素（kagome・[ADR 0018](adr/0018-morphological-tokenizer-as-measured-alternative.md)）は `RECALL_TOKENIZER=kagome` の正式な選択肢。同一条件の実測（[比較文書](benchmarks/2026-09-02-eval-morph-vs-bigram.md)）で `paraphrase` +0.18 / `orthography` −0.18 が相殺し、総合値と `MRR` に優位なし。`alpha=0.8` は両分割器のプラトーに残る。次は和集合分割器（bigram ∪ kagome 原形）を測る（予想は事前登録済み） |
