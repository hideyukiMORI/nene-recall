...
- **追従作業**:
  ...
  - ~~項目 8（SQLite）で BM25 系の数字が出たら、本 ADR の「却下した選択肢」の Go 側 BM25 の行に追記する~~ → 下の追記

## 追記（2026-09-02・SQLite との比較実測の後）

「測らずに却下した Go 側 BM25」の品質面の数字が、項目 8 の SQLite ストア（FTS5 `bm25()`）で出た。
正本は [`docs/benchmarks/2026-09-02-eval-store-comparison.md`](../benchmarks/2026-09-02-eval-store-comparison.md) 層2。

| 語彙のみ（`alpha=0.0`・rounds=5） | `recall@10` | `MRR` | micro |
| --- | ---: | ---: | ---: |
| Postgres `ts_rank`（正規化 0） | 0.620 | **0.736** | 126/236 |
| SQLite FTS5 `bm25()` | **0.649** | 0.718 | 135/236 |

- `recall@10` と `MRR` が**逆を向く**。+0.03 は「判断材料にしない」帯（±0.02〜0.04）の中。
- タグ別では `exact-term` +0.11 / `negation` −0.09。per-query は 46 同じ / 9 上昇 / 3 下降。
- 長文（>520字）の gold は両者 25/56 で**同数**——`bm25()` の利得は長文優遇ではない。

**判断: 本 ADR の Decision 1（`ts_rank`）は変えない。** 却下の理由は最初から構造（`org_id` の絞り込みが1箇所・
第二の状態を持たない）であり、品質面の数字はどちらの優位も示さなかった。BM25 系の採点関数に乗り換える
理由は今のところ無い。⚠️ ただし `exact-term` の +0.11 は、識別子の濃いコーパスでは BM25 の IDF が効く方向を
示している。一般文書での再測定（項目 7 のデータが入ったとき）でこの差が広がるなら、`ts_rank_cd` や
Postgres 側での IDF 相当の重み付けを候補として再検討する。
