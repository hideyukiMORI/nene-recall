...
## 追記（2026-09-02・和集合分割器の実測後）

Decision 3 の和集合分割器（`union:bigram+kagome:v1`・PR #24）を、bigram・kagome と同一セッションで測った
（[`2026-09-02-eval-union-tokenizer.md`](../benchmarks/2026-09-02-eval-union-tokenizer.md)・rounds=5・12 本）。
予想（[`2026-09-02-union-prediction.md`](../benchmarks/2026-09-02-union-prediction.md)・凍結）は**外れた**。

| 語彙のみ | bigram | kagome | union |
| --- | ---: | ---: | ---: |
| `recall@10` / `MRR` | 0.620 / **0.736** | **0.637** / 0.714 | 0.629 / 0.723 |
| `paraphrase` | 0.44 | **0.62** | **0.39** |
| `orthography` | 0.56 | 0.39 | 0.53 |
| ハイブリッド 0.8 `recall@10` / `MRR` | 0.724 / 0.807 | 0.731 / 0.786 | 0.734 / 0.807 |

- `orthography` は bigram の値に戻った（的中）が、**`paraphrase` は kagome に近づくどころか bigram をも下回った**。
  予想が挙げた 3 つの可能性のどれでもない。原形トークンの一致 1 件が、bigram トークンで膨らんだ分母（`ts_rank` の文書側レキシーム数・
  実バイト 1.29 倍）に薄められ、かつ kagome のレキシームの 74% が既に bigram 側にあるため「新しく拾える語」がほとんど無かった、と読める
- 総合値はすべて両者の**間**。ハイブリッド 0.8 は 0.734 で bigram との差 +0.010 はゆらぎ。`MRR` は下がった（二重加点の懸念は杞憂ではない）
- latency は bigram より遅く kagome とは分離しない

**判断: 和集合は採らない。既定は bigram のまま（Decision 1 のとおり）。union は計測用の実装として残す**（消す費用より残す費用が小さく、
`ts_rank` を IDF 付きの採点に替えたときの再測定候補になる）。**Q-2 は閉じたまま**。`paraphrase` の穴に効いた経路は kagome 単体だけで、
それを既定にすると `orthography` を失う。両取りは「トークンを足す」形ではできなかった。次に効くのは採点関数側（IDF）か reranker（Q-4）である。

**追従 2（2 モード問題）に追加の観察**: 本セッションの kagome の「低」モード（p50 5.9〜6.0ms）は前セッションの「高」帯と重なる。
⇒ **モードのラベルはセッション内でしか成立しない**。別セッションの latency を並べる比較は、当面いっさい行わない。
